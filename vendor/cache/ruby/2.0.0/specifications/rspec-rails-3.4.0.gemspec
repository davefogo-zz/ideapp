# -*- encoding: utf-8 -*-
# stub: rspec-rails 3.4.0 ruby lib

Gem::Specification.new do |s|
  s.name = "rspec-rails"
  s.version = "3.4.0"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib"]
  s.authors = ["David Chelimsky", "Andy Lindeman"]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\nF3MdtaDehhjC\n-----END CERTIFICATE-----\n"]
  s.date = "2015-11-12"
  s.description = "rspec-rails is a testing framework for Rails 3.x and 4.x."
  s.email = "rspec@googlegroups.com"
  s.homepage = "http://github.com/rspec/rspec-rails"
  s.licenses = ["MIT"]
  s.rdoc_options = ["--charset=UTF-8"]
  s.rubygems_version = "2.5.0"
  s.summary = "RSpec for Rails"

  s.installed_by_version = "2.5.0" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<activesupport>, ["< 4.3", ">= 3.0"])
      s.add_runtime_dependency(%q<actionpack>, ["< 4.3", ">= 3.0"])
      s.add_runtime_dependency(%q<railties>, ["< 4.3", ">= 3.0"])
      s.add_runtime_dependency(%q<rspec-core>, ["~> 3.4.0"])
      s.add_runtime_dependency(%q<rspec-expectations>, ["~> 3.4.0"])
      s.add_runtime_dependency(%q<rspec-mocks>, ["~> 3.4.0"])
      s.add_runtime_dependency(%q<rspec-support>, ["~> 3.4.0"])
      s.add_development_dependency(%q<rake>, ["~> 10.0.0"])
      s.add_development_dependency(%q<cucumber>, ["~> 1.3.5"])
      s.add_development_dependency(%q<aruba>, ["~> 0.5.4"])
      s.add_development_dependency(%q<ammeter>, ["= 1.1.2"])
    else
      s.add_dependency(%q<activesupport>, ["< 4.3", ">= 3.0"])
      s.add_dependency(%q<actionpack>, ["< 4.3", ">= 3.0"])
      s.add_dependency(%q<railties>, ["< 4.3", ">= 3.0"])
      s.add_dependency(%q<rspec-core>, ["~> 3.4.0"])
      s.add_dependency(%q<rspec-expectations>, ["~> 3.4.0"])
      s.add_dependency(%q<rspec-mocks>, ["~> 3.4.0"])
      s.add_dependency(%q<rspec-support>, ["~> 3.4.0"])
      s.add_dependency(%q<rake>, ["~> 10.0.0"])
      s.add_dependency(%q<cucumber>, ["~> 1.3.5"])
      s.add_dependency(%q<aruba>, ["~> 0.5.4"])
      s.add_dependency(%q<ammeter>, ["= 1.1.2"])
    end
  else
    s.add_dependency(%q<activesupport>, ["< 4.3", ">= 3.0"])
    s.add_dependency(%q<actionpack>, ["< 4.3", ">= 3.0"])
    s.add_dependency(%q<railties>, ["< 4.3", ">= 3.0"])
    s.add_dependency(%q<rspec-core>, ["~> 3.4.0"])
    s.add_dependency(%q<rspec-expectations>, ["~> 3.4.0"])
    s.add_dependency(%q<rspec-mocks>, ["~> 3.4.0"])
    s.add_dependency(%q<rspec-support>, ["~> 3.4.0"])
    s.add_dependency(%q<rake>, ["~> 10.0.0"])
    s.add_dependency(%q<cucumber>, ["~> 1.3.5"])
    s.add_dependency(%q<aruba>, ["~> 0.5.4"])
    s.add_dependency(%q<ammeter>, ["= 1.1.2"])
  end
end
