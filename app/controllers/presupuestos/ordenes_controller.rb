class Presupuestos::OrdenesController < ApplicationController
  ...
  # GET /ordenes/new
  def new
   @presupesto = Presupuesto.find(params[:presupuesto_id])
   @ordene = Ordene.new
  end
  ...
  def create
    ...
    @ordene.presupuesto = @ordene

    respond_to do |format|
      if @ordene.save
        format.html { redirect_to @presupuesto, notice: 'La orden fue creada exitosamente.' }
  ...
  private
    ...
    def ordene_params
      params.require(:ordene).permit(:fecha_orden, :medida, :unidad, :costo_unidad, :total, :presupuesto_id)
    end
end
