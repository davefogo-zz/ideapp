class Gasto < ActiveRecord::Base
	...
	def transaccion_proveedor_gasto
		Transaccion.create!(gasto_id: self.id, fecha: Time.now, debito: self.importe, subcuenta_puc_id: self.subcuenta_puc_id, numero_de_documento: self.proveedore.numero_de_documento)
	end

	def activo
		if compra_de_activo == true
			ActivoFijo.create!(fecha_de_compra: self.fecha_recepcion, gasto_id: self.id, importe: self.importe, numero_de_documento: self.proveedore.numero_de_documento)
		end
	end
	...
	def retenciones
		...
		unless self.ret_fte == 0
			...
			Transaccion.create!(gasto_id: self.id, fecha: Time.now, credito: self.importe_fte, subcuenta_puc_id: self.subcuenta_puc_id, numero_de_documento: self.proveedore.numero_de_documento)
		end
		unless self.ret_ica == 0
			self.importe_ica = importe * ret_ica
			self.subcuenta_puc_id = 717
			Transaccion.create!(gasto_id: self.id, fecha: Time.now, credito: self.importe_ica, subcuenta_puc_id: self.subcuenta_puc_id, numero_de_documento: self.proveedore.numero_de_documento)
		end
		if self.ret_iva == true
			self.importe_ret_iva = iva * 0.15
			self.subcuenta_puc_id = 717
			Transaccion.create!(gasto_id: self.id, fecha: Time.now, credito: self.importe_ret_iva, subcuenta_puc_id: self.subcuenta_puc_id, numero_de_documento: self.proveedore.numero_de_documento)
		end
		if self.iva_teo == true
			self.importe_iva_teo = iva * 0.15
			self.subcuenta_puc_id = 1716
			Transaccion.create!(gasto_id: self.id, fecha: Time.now, credito: self.importe_iva_teo, subcuenta_puc_id: self.subcuenta_puc_id, numero_de_documento: self.proveedore.numero_de_documento)
		end
	...
	def transaccion_iva_descontable_gasto
		...
		Transaccion.create!(gasto_id: self.id, fecha: Time.now, debito: self.iva, subcuenta_puc_id: self.subcuenta_puc_id, numero_de_documento: self.proveedore.numero_de_documento)
	end
