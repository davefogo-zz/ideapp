class PagoItem < ActiveRecord::Base
  ...
  def transaccion_pago_gasto
    ...
    Transaccion.create!(pago_id: self.pago.id, fecha: self.pago.fecha, credito: self.gasto.importe_menos_retenciones, subcuenta_puc_id: self.subcuenta_puc_id)
    self.subcuenta_puc_id = 692
    Transaccion.create!(pago_id: self.pago.id, fecha: self.pago.fecha, debito: self.gasto.importe_menos_retenciones, subcuenta_puc_id: self.subcuenta_puc_id)
  end
