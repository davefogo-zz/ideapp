class ReciboItem < ActiveRecord::Base
  ...
  after_save :generate_transaccion_recibo_item_debito_activo, :generate_transaccion_recibo_item_credito_activo
	  def generate_transaccion_recibo_item_debito_activo
	    ...
	    Transaccion.create!(recibo_de_caja_id: self.recibo_de_caja_id, fecha: Time.now, debito: self.importe, subcuenta_puc_id: self.subcuenta_puc_id, numero_de_documento: self.recibo_de_caja.cliente.numero_de_documento)
	  end

	  def generate_transaccion_recibo_item_credito_activo
	  	self.subcuenta_puc_id = 127
	    Transaccion.create!(recibo_de_caja_id: self.recibo_de_caja_id, fecha: Time.now, credito: self.importe, subcuenta_puc_id: self.subcuenta_puc_id, numero_de_documento: self.recibo_de_caja.cliente.numero_de_documento)
	  end
end
